...
ptw32_srwl_AllocReaderNode(ptw32_srwl_local_t* local)
{
  ...
  for (;;)
  {
	...
	currNodePtr = currNodePtr->qNextPtr; //nextReaderPtr
  }
}
...
{
	ptw32_srwl_rwlock_t *oldTailPtr = (ptw32_srwl_rwlock_t*)PTW32_INTERLOCKED_EXCHANGE_PTR(
  ...
  if (oldTailPtr != NULL)
  {
	...
	oldTailPtr->tailPtr->qNextPtr = localPtr->wNodePtr; // AD: could be oldTailPtr->readerNodePtr
    if (oldTailPtr->tailPtr->kind == ptw32_srwl_writer)// AD: could be oldTailPtr->readerNodePtr
    {
    ...
    else
    {
      ...
      while (ptw32_oll_Query(oldTailPtr->tailPtr->csnziPtr).open);// AD: could be oldTailPtr->readerNodePtr
      /*
       ...
       * critical section.
       */
      if (ptw32_oll_Close(oldTailPtr->readerNodePtr->csnziPtr)) // AD: could be oldTailPtr->tailPtr
      {
		  while (oldTailPtr->readerNodePtr->spin);
  ...
  for (;;)
  {
    ...
    if (tailPtr == NULL)
    {
      ...
      if (PTW32_INTERLOCKED_COMPARE_EXCHANGE_PTR(
          ...
          == (PTW32_INTERLOCKED_PVOID)rNodePtr)
      {
        ptw32_oll_Open(rNodePtr->csnziPtr);
        localPtr->ticket = ptw32_oll_Arrive(rNodePtr->csnziPtr);
    ...
    else
    {
      ...
      {
        localPtr->ticket = ptw32_oll_Arrive(tailPtr->csnziPtr);
        if (ptw32_oll_Arrived(localPtr->ticket))
        {
          if (rNodePtr != NULL)
          {
...
{
  if (ptw32_oll_Depart(/*localPtr->departFromPtr->csnziPtr,*/ localPtr->ticket))
  {
    return;
  }
  /* Clean up */
