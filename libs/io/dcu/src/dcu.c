...
Dcu_Err_t DCU_SetLayerTile(Dcu_Unit_t dcu_id, Dcu_TileLayer_t layer_id, Dcu_Tile_t* pTileData)
{
  ...
  if ((0x0800 > pTileData->mWidth) && (0x0800 > pTileData->mHeight))
  {
    ...
    if ((tile_addr > (DCU_BASE_ADDR32[dcu_id] + DCU_CLUT_OFFSET)) &&
        (tile_addr < (DCU_BASE_ADDR32[dcu_id] + DCU_CLUT_OFFSET + CLUT_SIZE)))
    {
      /* Set data select bit */
...
Dcu_Err_t DCU_CLUTSet(Dcu_Unit_t dcu_id, uint16_t aIndex, uint32_t aValue)
{
  ...
  {
    /* Get address of CLUT */
    vuint32_t* pCLUT = (vuint32_t*)REG_ADDR(DCU_BASE_ADDR32[dcu_id] + DCU_CLUT_OFFSET + mem_index);
    /* Set CLUT value */
    *(pCLUT) = aValue;
  }
  else
...
Dcu_Err_t DCU_CursorLoad(Dcu_Unit_t dcu_id, uint16_t aCount, const uint32_t* apcValues)
{
  Dcu_Err_t err = DCU_ERR_OK;
  uint32_t i;
  vuint32_t* pCursor = (vuint32_t*)REG_ADDR(DCU_BASE_ADDR32[dcu_id] + DCU_CURSOR_OFFSET);
