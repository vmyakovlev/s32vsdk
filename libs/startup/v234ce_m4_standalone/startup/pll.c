...
extern void mode_enter(uint16_t mode);
...
int i=0;
  ...
  while(DFS_0.PORTSR.R != 0xF);	//DFS0_PORTSR port status register - wait until DVPORT 1-3 are locked, DVPORT 4 not implemented
    ...
    MC_ME.DRUN_MC.B.SYSCLK = 0x0002;	// system clk is ARM_PLL
       MC_ME.DRUN_SEC_CC_I.B.SYSCLK1 =2; //ARMPLL as system clock1
...
for(i=0;i<100000;i++); 
    ...
    mode_enter(DRUN_M);
    while(MC_ME.GS.B.S_PERIPHPLL != 1);
...
for(i=0;i<100000;i++); 
    ...
    ///////////////////////////////////////////////////////
/*        // PLL settings
 -     SRC.GPR1.R |= 0x20000000;
 -    // PLLDIG_3.PLLCAL3.R=0x14000000;
 -     PLLDIG_3.PLLDV.R=0x0202101a;
 -     PLLDIG_3.PLLFD.R=0x70003400;
 -     DFS_2.DVPORT[0].R=0x00000221;
 -     DFS_2.DVPORT[1].R=0x00000221;
 -     DFS_2.DVPORT[2].R=0x0000030b;
 -    // DFS_2.DVPORT[3].R=0x00000202;
 -     DFS_2.CTRL.R &= 0xfffffffd;
 -     DFS_2.PORTRESET.R &= 0xfffffff0;
 - //ME_RUN0_MC_GENERIC) M=0x00000200
 - 
 -     MC_ME.MCTL.R = 0x30005AF0;
 -     MC_ME.MCTL.R = 0x3000A50F;		
 -     while(MC_ME.GS.B.S_MTRANS == 1);
 -     
 -     //MC_CGM0 AUX registers
 ...
 -     MC_ME.MCTL.R = 0x30005AF0;
 -     MC_ME.MCTL.R = 0x3000A50F;		// Turn ON the DDRPLL in DRUN mode
 -     while(MC_ME.GS.B.S_MTRANS == 1);
...
//PLLDIG_4.PLLCAL3.R=0x14000000; // validation team using it but this register is not in RM!
    *((vuint32_t*)(0x4003c218)) = 0x14000000;
    ...
    PLLDIG_4.PLLDV.B.MFD = 26;	// Loop multiplication factor divider
...
//       PLLDIG_4.PLLFD.B.MFN = 0x4000;	//FRCDIV value - numerator for fractional loop division factor
        *((vuint32_t*)(0x4003c230)) = 0x70004000;
...
void clkout_settings (uint32_t clkout_value){
    uint32_t clk_in = 400;
     ...
     SIUL2.MSCR[39].B.MUX_MODE = 0x2;		// source signal select
